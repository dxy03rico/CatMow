"""
record_yowl.py
录制猫嚎叫（yowl）声音，用于训练新模型的 yowl 类别

使用方法：
  python record_yowl.py              # 每段默认 90 秒
  python record_yowl.py --duration 60 # 每段 60 秒
  python record_yowl.py --list        # 查看音频设备

录制建议：
  - 在手机/电脑上播放猫嚎叫视频，让 Pi5 麦克风拾音
  - 每段 90 秒，录 8-10 段（合计约 12-15 分钟）
  - 可以播放不同类型的嚎叫：受惊/发情/打架/痛苦
...
SAVE_DIR    = Path(__file__).parent / "yowl_recordings"
SAMPLE_RATE = 44100
CHANNELS    = 1
...
def record_segment(seg_idx: int, duration: int, device=None) -> Path:
    """录制一段 yowl 音频，使用 sd.rec() 简单可靠方式"""
    SAVE_DIR.mkdir(parents=True, exist_ok=True)
    out_path = SAVE_DIR / f"yowl_{seg_idx:02d}.wav"
    ...
    print(f"  ─────────────────────────────────────────")
    print(f"  现在开始播放嚎叫视频，录音中...（Ctrl+C 提前结束并保存）\n")
    ...
    print(f"     有效内容占比: {active_pct:.0f}%  "
          f"({'充足 ✓' if active_pct >= 30 else '偏少，建议多播放一些嚎叫声 ⚠️'})")
...
def main():
    parser = argparse.ArgumentParser(description="CatMow yowl 录音工具")
    ...
    print("=" * 50)
    print("  CatMow  嚎叫（yowl）录音工具")
    print("=" * 50)
    print(f"\n  每段时长：{args.duration} 秒")
    print(f"  目标段数：8-10 段（合计 ≥12 分钟）")
    print(f"  保存目录：{SAVE_DIR}")
    print()
    print("  可搜索播放：cat yowling / cat screaming / cats fighting")
    print("  Ctrl+C 一次：提前结束当前段并保存")
    print("  Ctrl+C 两次：退出程序")
    print()

    existing = sorted(SAVE_DIR.glob("yowl_*.wav")) if SAVE_DIR.exists() else []
    ...
    all_files = sorted(SAVE_DIR.glob("yowl_*.wav"))
